    ...
    if (result.error) {
      ...
      if (facilities.length == 0) {
        ...
        facilities = await getSelectedColumn(
          "`dbo.facilities` F ",
          where,
          "F.sub_group_id,F.ID as facilityID ,F.AssestName as name"
        );
        ...
        facilities.map(async (item) => {
          ...
          if (item?.group_id != 0 && item?.group_id != undefined) {
            ...
            const groupmap = await getSelectedColumn(
              "`dbo.group` G",
              where2,
              " G.groupname as AssestName,G.tenantID,G.id,G.is_subgroup"
            );
            if (groupmap?.length > 0) {
              if (groupmap[0]?.is_subgroup == 0) {
                where1 =
                  " JOIN  `dbo.facilities` F ON G.facilityId = F.ID where G.groupId = '" +
                  item.group_id +
                  "'";
                facilities1 = await getSelectedColumn(
                  "`dbo.groupmapping` G",
                  where1,
                  "G.facilityId,F.ID as id ,F.AssestName as AssestType,F.AssestType as name"
                );
              } else {
                ...
                facilities1 = await getSelectedColumn(
                  "`dbo.facilities` F ",
                  where,
                  "F.ID as id ,F.AssestName as AssestType,F.AssestType as name"
                );
              }
            }
          } else {
  ...
  try {
    ...
    if (result.error) {
      ...
      let user = {
        ...
        tenantID: tenantID ? tenantID : facilities[0].tenantID,
  ...
  try {
    ...
    productArray = await Promise.all(
      productArray.map(async (product) => {
        const productResult = await getPurchaseCategoriesEf(product['Product Description']);
...
exports.getAllPurchaseCategoriesEf = async (req, res) => {
  try {
    const getAllPurchaseResult = await getAllPurchaseCategoriesEf();
