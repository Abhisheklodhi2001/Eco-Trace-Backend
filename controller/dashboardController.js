...
const {
  ...
  getscope1CombustionEmissionDetail
  ...
  try {
    ...
    if (result.error) {
      ...
      if (UserFacility.length == 0) {
        ...
        var facilities = await getSelectedColumn(
          "`dbo.facilities` F ",
          where,
          "F.ID,F.AssestName as AssestType,F.AssestType as name, F.TenantID as tenantID,F.ID as id"
        );
      } else if (UserFacility[0].facilityID == 0) {
        let where = "  where  F.TenantID = '" + tenantID + "'";
        var facilities = await getSelectedColumn(
          "`dbo.facilities` F ",
          where,
        ...
        const numbers = UserFacility[0].facilityID
          .split(",")
          .map((num) => num.trim());
        const formattedNumbers = `('${numbers.join("', '")}')`;

        let where = "  where  F.ID IN " + formattedNumbers;
        var facilities = await getSelectedColumn(
          "`dbo.facilities` F ",
          where,
          "F.ID,F.AssestName as AssestType,F.AssestType as name, F.TenantID as tenantID,F.ID as id"
      ...
      console.log(facilities, "facilitiesfacilities");
      if (facilities.length > 0) {
        let where1 =
          ' where G.tenantID = "' +
          tenantID +
          '"' +
          " AND G.is_subgroup <> 1  AND G.is_main_group <> 1 AND G.group_by <> 2";
        const groupmap = await getSelectedColumn(
          "`dbo.group` G",
          where1,
          " G.groupname as name,G.tenantID,G.id"
        );
    ...
    if (result.error) {
      ...
      if (groupmap.length > 0) {
        ...
        let subGroups = await getSelectedColumn(
          "`dbo.group` F ",
          where2,
          "F.id ,F.country_id,F.is_main_group,F.groupname as name,F.tenantID,C.Name as country_name,F.is_subgroup"
        );
