...
exports.purchaseGoodsAllcategories = async (req, res) => {
  try {
    const { product_code_id, typeofpurchase } = req.body;
    const schema = Joi.alternatives(
      Joi.object({
        product_code_id: [Joi.string().empty().required()],
          ...
          if (sub_categories.length > 0) {
            ...
            await Promise.all(
              sub_categories.map(async (item1) => {
                const categoriesproduct = await fetchPurchase_categoriesproduct(product_code_id, typeofpurchase, item1.value, item.value);
