  ...
  try {
    ...
    if (result.error) {
      ...
      if (allemployeescommute.length > 0) {
        return res.json({
          success: false,
          message: "employee commuting category is already added to this facilities in this year",
          status: 400,
        });
      }
            ...
            if (getemployeecommuting.length > 0) {
              ...
              if (year >= startYear && year <= endYear) {
                if (item.employeesCommute && item.avgCommute) {
                  ...
                  emission_kg = parseFloat(item.employeesCommute * item.avgCommute * totatotalnoofcommutes * getemployeecommuting[0]?.efskgCO2e)
                  ...
                  emission_kg = parseFloat(item.employeesCommute * totatotalnoofcommutes * getemployeecommuting[0]?.efskgCO2e)
                } else {
  ...
  try {
    ...
    if (result.error) {
      ...
      if (homeofficecategory_exits.length > 0) {
        return res.json({
          success: false,
          message: "home office category of this current Year is already inserted!",
          status: 400,
        });
      }
      let array = [];
      ...
      if (countrydata.length == 0) {
        return res.json({
          success: false,
          message: "EF not Found  while Adding homeoffice category",
          status: 400,
        });
      }
      ...
      if (typeofhomeoffice) {
        ...
        await Promise.all(
          json.map(async (item) => {
            var emission_kg = 0;
            const assumptions = await homeoffice_emission_factors(item.type, countrydata[0].CountryId);
    ...
    if (result.error) {
      ...
      if (valueofsoldintermediate) {
        ...
        if (processing_acitivity && intermediate_category) {
          ...
          where = ` where sub_sector = '${sub_activity}' and country_id = '${countrydata[0].CountryId}'`;
          getSoldproduct = await getSelectedColumn("processing_of_sold_products_ef", where, "*");
        ...
        if (getSoldproduct.length > 0) {
          ...
          return res.json({
            success: false,
            message: "EF not Found for processing of sold products category",
            status: 400,
          });
        }
