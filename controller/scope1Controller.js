...
exports.electricitygridType = async (req, res) => {
  try {
    ...
    if (result.error) {
      ...
      let where = ` where SubCategorySeedID = '9' and country_id = '${countrydata[0].CountryId}' and is_scope3 != 1`;
      ...
      if (electricity.length > 0) {
        ...
        return res.json({
          success: false,
          message: "Some problem occured while selecting all Subcategory Type",
          status: 400,
        });
      }
    }
...
exports.GetSubCategoryTypes = async (req, res) => {
  try {
    ...
    if (result.error) {
      ...
      if (id == '7') {
        let where = ` where SubCategorySeedID = '${id}' AND country_id = '${countrydata[0].CountryId}'`;
...
exports.addMultipleCompanyOwnedVehicles = async (req, res) => {
  try {
    ...
    if (result.error) {
      ...
      if (allinsertedID.length > 0) {
        let where = ` where user_id ='${user_id}' and facilities = '${facilityId}'`;
        const refrigents = await getSelectedColumn("`dbo.vehiclede`", where, "*");
        return res.json({
          success: true,
          message: "Succesfully Added company owned vehicles",
